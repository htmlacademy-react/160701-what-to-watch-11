...
const FilmCard = ({ film }: TFilmCard) => {
  ...
  type TPropsWithChildren<P = unknown> = P & { children: JSX.Element };

  const FilmCardHeroWrap = ({ children }: TPropsWithChildren) =>
    isMoviePage ? <div className="film-card__hero">{children}</div> : children;

  const FilmCardHeaderWrap = ({ children }: TPropsWithChildren) =>
